...
> God node · 20 connections · [C:\WeatherGPT-068\backend\models\schemas.py](file:///C:/WeatherGPT-068/backend/models/schemas.py#L35)

## Call Trace Diagram

```mermaid
sequenceDiagram
    participant P0 as WeatherResponse
    participant P1 as WeatherGPT — FastAPI ASGI Application. Central orchestration layer serving REST
    participant P2 as ChatQuery
    ...
    participant P16 as AIChatService
    participant P17 as test_chat_spray_query_intent()
    participant P18 as Conversational AI Service. Integrates Grounded Intent Parsing, Spatial Semantic
    participant P19 as test_spatial_cache_deduplication()
    participant P20 as MissedCallRequest
    ...
    participant P27 as WeatherService
    participant P28 as ._parse_open_meteo_response()
    participant P29 as ._get_fallback_weather()
    participant P30 as Live Meteorological Ingestion Service. Fetches high-resolution GFS/ECMWF numeric
    P0->>+ P1: uses
    P1-->>- P0: return
    P1->>+ P0: uses
    P0-->>- P1: return
